...
def prepare_scatter_data(df_monthly: pd.DataFrame,
    ...
    scatter_df = scatter_df.dropna(subset=["winter_retention", "volume_per_sensor"])
...
#     agg_dfs = split_by_agg(raw_data)
#     viz2_heatmap = build_viz2_heatmap_data(agg_dfs["h"])
#     viz2_heatmap.to_csv(VIZ2_OUTPUT_PATH, index=False)


#     agg_dfs['h'].to_csv(HOURLY_PATH, index=False)  # for scatter viz prep
#     agg_dfs['m'].to_csv(MONTHLY_PATH, index=False)  # for scatter viz prep
